...
def main(args):
    ...
    for i, (input, target) in enumerate(train_bar):
        ...
        loss.backward()
        grad_learn = input.grad
        sign_learn = grad_learn.sign()
        sign_query = grad_query.sign()
        same_num = (sign_learn == sign_query).sum(dim=(1,2,3))
        input.grad.zero_()
        print(same_num/(grad_learn.shape[1]*grad_learn.shape[2]*grad_learn.shape[3]))
